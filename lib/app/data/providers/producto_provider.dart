...
class CategoriaProvider {
  ...
  await model.categoriaExists(name)? print('Ya existe esa categoria'): await model.addCategoria(name);
  }

  static void delateCategoria(String id) async{
    Categoria? categoria = await model.getCategoria(id);
    categoria != null ? categoria.active = false: print('No existe la categoria');
  }

  static void updateCategoria(String id, String nombre) async{
    Categoria? categoria = await model.getCategoria(id);
    categoria != null ? await model.updateCategoria(id, Categoria(id: id, nombre: nombre)): print('No existe la categoria');
  }
